...
class AllArticles(scrapy.Spider):
    ...
    start_urls = [
        "http://www.banyuetan.org/chcontent/gd/pl/index.shtml"
    ]

    # 爬取第一页，传递给parse_page()
    def parse(self, response):
        page = 1
        url = "http://www.banyuetan.org/chcontent/gd/pl/index.shtml"
        yield scrapy.Request(url, callback=self.parse_page)
        if page >= 2:
            for r in range(2, page + 1):
                url = "http://www.banyuetan.org/chcontent/gd/pl/index_" + str(r) + ".shtml"
                yield scrapy.Request(url, callback=self.parse_page)
